...
* CROSとは何の省略ですか？また、それはどんな問題を表しますか？
...
* `<script>`、`<script async>`、`<script defer>`の違いを述べて下さい。
* 何故、一般的にCSSの `<link>`は`<head></head>`の内に、JSの`<script>`を`</body>`直前に記述するのがよいとされていますか？例外はありますか？
* プログレッシブレンダリングとは何ですか？
* なにかHTMLテンプレート言語を使用したことはありますか？
